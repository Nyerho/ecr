...
export default function Dispatch() {
  ...
  ), [incidents, statusFilter, categoryFilter, priorityFilter]);
  ...
  function updateIncident(changes: Parameters<typeof updateSharedLocalIncident>[1]) {
    if (!selected) return;
    const updated = updateSharedLocalIncident(selected.id, changes);
    if (!updated) return;
